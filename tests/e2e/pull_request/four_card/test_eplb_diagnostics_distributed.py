...
def _worker(rank, rendezvous):
    ...
    try:
        ...
        config = EplbDiagnosticsConfig(mode="observe", window_size=2, warmup_steps=0, max_windows=2)
        recorder = DiagnosticsRecorder(config, [("layer", layer)], group, [rank], stage, source)
        output = io.StringIO()
        ...
        try:
            for _ in range(2):
                for tokens, dummy in ((3, False), (2, local == 1)):
                    ...
                    recorder.end()
            if local == 0:
                assert output.getvalue().count("[EPLB diagnostic]") == 2
