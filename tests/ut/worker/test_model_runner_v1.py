...
class TestNPUModelRunnerNaNDetection(unittest.TestCase):
    """NaN-in-logits detection wired through _bookkeeping_sync.

    Mirrors upstream vllm/v1/worker/gpu_model_runner.py: when
    VLLM_COMPUTE_NANS_IN_LOGITS is enabled, _bookkeeping_sync must return
    per-request NaN counts as the first element of its result tuple, so the
    scheduler can flag corrupted requests (vllm:corrupted_requests metric).
    """
    ...
    def test_bookkeeping_sync_returns_per_request_nan_counts_first(self):
        ...
        # NaN counts come back as the first tuple element, keyed by request id.
        self.assertEqual(len(result), 7)
        self.assertEqual(result[0], {"req-0": 2, "req-1": 0, "req-2": 3})
        # The rest of the tuple keeps its contract.
        self.assertIsNone(result[1])  # logprobs_lists
        self.assertEqual(result[2], [[1], [2], [3]])  # valid_sampled_token_ids
        self.assertEqual(result[4], ["req-0", "req-1", "req-2"])  # req_ids copy
    ...
    def test_bookkeeping_sync_defaults_to_zero_when_req_has_no_logits_row(self):
        # req_ids can outlive the sampled rows (e.g. aborted requests still
        # resident in input_batch); counts must degrade to 0 instead of crashing.
        runner = self._build_runner(num_reqs=3)
        logits = torch.tensor(
            [
                [float("nan"), 1.0],
